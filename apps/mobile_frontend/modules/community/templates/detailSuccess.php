<?php sa_mobile_page_title($community->getName(), __('Detail of %community%')) ?>
...
$list = array(
  ...
  __('Date Created')       => sa_format_date($community->getCreatedAt(), 'D'),
  __('Administrator')      => sa_link_to_member($communityAdmin),
);
$subAdminCaption = array();
foreach ($communitySubAdmins as $m)
{
  $subAdminCaption[] = sa_link_to_member($m);
}
...
$list[__('%community% Description', array(), 'form_community')] = sa_auto_link_text_for_mobile(nl2br($community->getConfig('description')));
?>
<?php foreach ($list as $key => $value): ?>
<font color="<?php echo $sa_color["core_color_19"] ?>"><?php echo $key ?>:</font><br>
<?php echo $value ?><br>
<?php endforeach; ?>
