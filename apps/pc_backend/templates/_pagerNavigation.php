...
<?php echo sa_link_to_for_pager($options['prev_text'], $sf_data->getRaw('internalUri'), $pager->getPreviousPage(), $options->getRaw('link_options')) ?>&nbsp;
<?php endif; ?>
<?php if ($options['is_total']): ?>
<?php sa_include_pager_total($pager) ?>
<?php endif; ?>
<?php if ($pager->getNextPage() != $pager->getPage()): ?>
&nbsp;<?php echo sa_link_to_for_pager($options['next_text'], $sf_data->getRaw('internalUri'), $pager->getNextPage(), $options->getRaw('link_options')) ?>
<?php endif; ?>
<?php endif; ?>
